...
const HeaderForm = ({
  ...
  const sampleFeaturesFilter = (features) => {
    const nonChoiceFeatures = features.filter((feature) => {
      return (!(feature.source.split(":")[3]==="choice"))
    });
    return nonChoiceFeatures;
  }
  ...
  const handleSetClass = (e) => {
    ...
    if (e.target.value === "Barbarian") {
      ...
      setFeatures(sampleFeaturesFilter(features.Barbarian));
      ...
      setFeatures(sampleFeaturesFilter(features.Fighter));
      ...
      setFeatures(sampleFeaturesFilter(features.Rogue));
    } else {
      setHitpoints(8 + Math.floor((attributes.con - 10) / 2));
      setProfChoices([
        "Athletics",
      ...
      setFeatures(sampleFeaturesFilter(features.Monk));
    }
  };
